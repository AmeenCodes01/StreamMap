...
function Sessions() {
  ...
  const { inSesh, setInSesh, seshInfo, setSeshInfo, seshGoal,setSeshGoal, showRating , setShowRating} = useTimeContext();
...
const calculateSessionScore = (duration, rating) => {
  ...
  const durationWeight = 1.4; // Adjust as needed
  const ratingWeight = 1.6; // Adjust as needed

  // Calculate the score
  const durationScore = duration / 60 * durationWeight; // Convert duration to hours
  const ratingScore = rating * ratingWeight;

  // Combine duration and rating scores
  const totalScore = durationScore + ratingScore;
    ...
    <div className="w-[100%] justify-items-end">
      <div className="flex flex-col mb-[20px]  ">
        ...
        <div className="flex md:flex-row flex-col ml-auto  ">
          <p className="flex flex-row w-[300px] text-end self-center justify-self-end font-light tracking-wide font-serif font-[16px]">
        ...
        {showRating? (
          <div className="flex flex-row mt-[10px] border-[10] ml-[auto] mr-[50px] ">
            ...
            <input
              ...
              onChange={(e) => {
                const regex = /^[0-9\b]+$/;
                if ((e.target.value === "" || regex.test(e.target.value)) && e.target.value <  11  ) {
                  e.target.value < 11 ?
                  setSeshRating(e.target.value) : null
                }
              }}
            />
